...
export const CreateCustomer = ({
  ...
  const handleCreateCustomer = () => {
    ...
    if (allRequiredFilledsNotEmpty) {
      setLoading(true);
      createCustomer(data).then((resp) => {
        setLoading(false);
        if (resp?.status === 201) {
          onCreatedCustomer && onCreatedCustomer(resp?.data);
          cogoToast.success("Customer has been created successfully", {
            hideAfter: 3,
            position: "top-right",
          });
          handleClose();
        } else {
          cogoToast.error("Error, customer has not been created", {
            hideAfter: 3,
            position: "top-right",
          });
        }
  ...
  const handleEditUser = () => {
    ...
    if (allRequiredFilledsNotEmpty) {
      setLoading(true);
      editCustomer(data).then((resp) => {
        setLoading(false);
        if (resp?.status === 200) {
          onUpdateCustomer && onUpdateCustomer(resp?.data);
          cogoToast.success("Customer has been updated successfully", {
            hideAfter: 3,
            position: "top-right",
          });
          handleClose();
        } else {
          cogoToast.error("Error, customer has not been updated", {
            hideAfter: 3,
            position: "top-right",
          });
        }
  ...
  return (
    <Modal
      ...
      onSave={() =>
        selectedCustomer ? handleEditUser() : handleCreateCustomer()
      }
      loading={loading}
    >
      <StyledCreateCustomer>
        ...
        activeTab.value === "More" ? (
          <More data={data} onUpdateData={handleUpdateData} errors={errors} />
        ) : null}
      </StyledCreateCustomer>
    </Modal>
  );
